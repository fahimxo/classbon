...
import CoursesCardList from "./(courses)/_components/course-card-list";
...
async function getNewestBlog(count: number): Promise<BlogPostSummery[]> {
...
export default async function Home() {
  ...
  const newestBlogData = getNewestBlog(4);

  const [newestBlog] = await Promise.all([
    // newestCoursesData,
    newestBlogData,
  ]);
  return (
    <>
      <HomeHeroSection />
      <section className="container dark:bg-base-75 mt-10">
        <div className="flex flex-col xl:flex-row py-10 gap-10 xl:gap-5">
      ...
      <section className="container pt-20">
        <div className="text-center xl:text-right">
          ...
          <p>برای بروز موندن، یاد گرفتن نکته های تازه ضروریه</p>
        </div>
        <Suspense fallback={<CardPlaceholder count={4} className="mt-5" />}>
          <CoursesCardList courses={[]} />
        </Suspense>
      </section>
      ...
      <section className="container py-20">
        ...
        <BlogPostCardList blogs={newestBlog} />
      </section>
